...
＊「ブロックリスト」として自分がブロックするユーザのリストを共有/ワンタップでブロック適用できる機能

## 仕様(仕組み)

P2P方式でデータを各ノードが分散保持(/globalにはalgorithm.rbで選定した最新ポストをN件表示)
ポストなどのデータはCIDで静的に、~~更新系のものはGossipプロトコルを用いて拡散更新・管理する~~
必要な時に他ノードの/latest_postsにGETで取得
Webrickを用いてページをレンダリング、~~TCPサーバーを平行して起動し更新系データを伝播~~
必要なかったし面倒なやり方なのでHTTPサーバーに統一

ユーザーのデータはusers/中の各ユーザーフォルダに保管
新規登録の際にusers/templateを複製し初期データとする
同時にusername始めとするプロフィールデータを保存
postフォルダにポストを格納し、フォロワーノードなどにこのフォルダをコピーする